...
function normalizeUsageDocument(document = {}) {
  if (!document) {
    return null;
  }

  return {
    $id: document.$id || document.documentId || document.document_id || "",
    documentId: document.documentId || document.document_id || document.$id || "",
    userId: document.userId || document.user_id || "",
    tenantId: document.tenantId || document.tenant_id || "",
    planId: document.planId || document.plan_id || "",
    metric: document.metric || "",
    periodType: document.periodType || document.period_type || "",
    periodStart: document.periodStart || document.period_start || "",
    used: Number(document.used || 0),
    limit: document.limit ?? document.usage_limit ?? null,
    status: document.status || PLAN_USAGE_STATES.NORMAL,
    resetAt: document.resetAt || document.reset_at || "",
...
function usageRowFromPayload(payload = {}) {
  return {
    document_id: payload.documentId,
    user_id: payload.userId,
    tenant_id: payload.tenantId || "personal",
    plan_id: payload.planId,
    metric: payload.metric,
    period_type: payload.periodType,
    period_start: payload.periodStart,
    used: Number(payload.used || 0),
    usage_limit: payload.limit,
    status: payload.status,
    reset_at: payload.resetAt || null,
...
async function getUsageDocument({ userId, metric, periodType, start }) {
  ...
  if (supabaseConfigured) {
    ...
    const { data, error } = await supabase
      .from("nexa_plan_usage")
      .select("*")
      .eq("document_id", documentId)
      .maybeSingle();
    throwSupabaseError(error, "Failed to load Supabase plan usage.");
    return normalizeUsageDocument(data);
  }
  ...
  try {
    return normalizeUsageDocument(await databases.getDocument(
      databaseId,
      planUsageCollectionId,
      documentId,
    ));
...
export async function recordPlanUsage(userId, metric, amount = 1) {
  if ((!planUsageCollectionId && !supabaseConfigured) || !userId || !metric || !amount) {
    return [];
  }
    ...
    if (supabaseConfigured) {
      try {
        ...
        const payload = {
          documentId,
          userId,
          tenantId: current?.tenantId || "personal",
          planId,
          metric,
          periodType,
          periodStart: start,
        ...
        const { data, error } = await supabase
          .from("nexa_plan_usage")
          .upsert(usageRowFromPayload(payload), { onConflict: "document_id" })
          .select()
          .single();
        throwSupabaseError(error, "Failed to record Supabase plan usage.");
        updates.push(normalizeUsageDocument(data));
      } catch (error) {
...
export async function setPlanUsage(userId, metric, usedValue = 0) {
  if ((!planUsageCollectionId && !supabaseConfigured) || !userId || !metric) {
    return null;
  }
  ...
  if (supabaseConfigured) {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select()
        .single();
      throwSupabaseError(error, "Failed to set Supabase plan usage.");
      return normalizeUsageDocument(data);
    } catch (error) {
