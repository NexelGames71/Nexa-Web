...
export async function createBackendImageJob(args) {
  const response = await fetch(`${IMAGE_BACKEND_URL}/v1/image/jobs`, {
    method: "POST",
...
export async function pollBackendImageJob(backendJobId) {
  const response = await fetch(`${IMAGE_BACKEND_URL}/v1/image/jobs/${encodeURIComponent(backendJobId)}`, {
    cache: "no-store",
  });
...
export async function waitForBackendImageResult(args) {
  ...
  while (Date.now() - startedAt < IMAGE_JOB_TIMEOUT_MS) {
    ...
    if (job.status === "completed" && job.result) {
      return normalizeGeneratedImagePayload(job.result);
    }
  }
...
export function normalizeGeneratedImagePayload(payload) {
  const root = payload && typeof payload === "object" ? payload : {};
  const nestedImage = root.image && typeof root.image === "object" ? root.image : {};
  const nestedData =
    root.data && !Array.isArray(root.data) && typeof root.data === "object" ? root.data : {};
  const openAiImage =
  ...
  const rawB64 =
    ...
    "";
  let b64 = rawB64;
  let mimeType = merged.mime_type || merged.mimeType || root.mime_type || "image/png";
  ...
  return {
    ...root,
    ...merged,
    url: typeof url === "string" && !url.startsWith("data:image") ? url : "",
    b64: typeof b64 === "string" ? b64 : "",
    mime_type: mimeType,
    prompt_used:
      merged.prompt_used ||
      merged.promptUsed ||
      merged.revised_prompt ||
      merged.prompt ||
      root.prompt_used ||
      "",
  };
...
export async function persistGeneratedImageAsset(imagePayload, { userId, prompt }) {
  imagePayload = normalizeGeneratedImagePayload(imagePayload);
...
export async function finalizeGeneratedImageJob({ job, imagePayload }) {
  ...
  const title = meta.needsGeneratedTitle
    ? buildConversationTitle(content || prompt)
