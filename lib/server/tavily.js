...
function buildTavilyGroundingPrompt({ query, answer, sources }) {
  ...
  return [
    ...
    `Search query: ${query}`,
    tavilyAnswer.trim(),
    "These are the authoritative sources for this answer.",
    "Use these sources for current facts and ignore older model memory when it conflicts with them.",
    "If the sources do not support a claim, say so instead of guessing.",
    "Mention source names naturally in the answer when using web facts. Do not expose internal search metadata.",
    sourceLines,
  ]
    .filter(Boolean)
    .join("\n\n");
...
export async function searchTavily(query, options = {}) {
  ...
  try {
    ...
    const searchDepth = options.searchDepth || process.env.TAVILY_SEARCH_DEPTH || DEFAULT_SEARCH_DEPTH;
