...
function toTitleCase(value) {
  const uppercaseWords = new Set(["ai", "api", "ui", "ux", "ml", "pc", "gpu", "cpu", "rgb"]);
...
export function buildConversationTitle(input) {
  ...
  if (
    /^(hi|hello|hey|yo|sup|hola)\b/.test(lower) &&
    (
      normalized.split(/\s+/).length <= 5 ||
      /\b(what'?s popping|what'?s up|how are you|how are you doing|how'?s it going)\b/.test(lower)
    )
  ) {
    return DEFAULT_CONVERSATION_TITLE;
  }
  ...
  if (/\bwhat can you do\b/.test(lower)) {
    return "Nexa Capabilities";
  }

  const imageIntent =
    /\b(create|generate|draw|paint|render|design|make|illustrate|visualize|visualise)\b.{0,90}\b(image|picture|photo|illustration|artwork|logo|poster|wallpaper|avatar|icon|sticker|scene|concept)\b/.test(
      lower,
    );
  if (imageIntent) {
    const imageSubject = normalized
      .replace(/^(please\s+)?(create|generate|draw|paint|render|design|make|illustrate|visualize|visualise)\s+/i, "")
      .replace(/\b(an?|the|my|custom)\b/gi, " ")
      .replace(/\b(image|picture|photo|illustration|artwork|poster|wallpaper|scene)\b/gi, " ")
      .replace(/\b(concept)\b/gi, "Concept")
      .replace(/\s+/g, " ")
      .trim();
    if (imageSubject) {
      return truncate(toTitleCase(imageSubject.split(/\s+/).slice(0, 5).join(" ")), 40);
    }
  }
