...
function isFreshActiveJob(job) {
  if (!isActiveStatus(job?.status)) {
    return false;
  }

  const updatedAtMs = parseJobTimeMs(job.updatedAt) || parseJobTimeMs(job.createdAt);
  if (!updatedAtMs) {
    return true;
  }

  return Date.now() - updatedAtMs <= ACTIVE_JOB_STALE_MS;
}

function jobMatchesKind(job, kind) {
  if (!kind || kind === "all") {
    return true;
  }

  const jobKind = String(job?.meta?.kind || "image").toLowerCase();
  if (kind === "image") {
    return jobKind !== "video";
  }

  return jobKind === String(kind).toLowerCase();
}

function staleJobUpdatePayload(document) {
  const now = nowIso();
  return {
    userId: document.userId,
    conversationId: document.conversationId,
    status: "failed",
    progress: Number(document.progress || 0),
    title: document.title || "Image generation expired",
    detail: "This image generation job expired before it finished. Start a new image request.",
    aspectRatio: document.aspectRatio || "1:1",
    style: document.style || "image design",
    resultJson: document.resultJson || "",
    error: "Image generation job expired before completion.",
...
export async function countActiveImageGenerationJobs(userId, options = {}) {
  cleanupExpiredJobs();
  ...
  for (const [id, job] of store.jobs.entries()) {
    ...
    if (!jobMatchesKind(job, kind)) {
      continue;
    }

    if (isFreshActiveJob(job)) {
      count += 1;
      continue;
    }
  ...
  try {
    ...
    for (const document of result.documents || []) {
      const job = serializeJobDocument(document);
      if (!jobMatchesKind(job, kind)) {
        continue;
      }
      ...
      staleDocuments.map((document) =>
        databases
          .updateDocument(
            ...
            imageGenerationJobsCollectionId,
            document.$id,
            staleJobUpdatePayload(document),
          )
          .catch((error) => {
            console.warn("Stale image job cleanup failed:", error?.message || error);
          }),
      ),
    );
