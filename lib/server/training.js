...
import { createSignedDownloadUrl, r2TrainingExportsBucketName, uploadR2Object } from "./r2";
...
async function uploadTrainingArtifacts({ exportId, exportName, scope, mode, prefix, examples, optedInPreferences, watermark, totals }) {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    await uploadR2Object({
      bucketName: r2TrainingExportsBucketName,
      bucketEnvName: "R2_TRAINING_EXPORTS_BUCKET_NAME",
      key: objectKey,
      body,
  ...
  const files = await Promise.all(
    exportJob.files.map(async (file) => ({
      ...
      url: await createSignedDownloadUrl(
        ...
        "R2_TRAINING_EXPORTS_BUCKET_NAME",
      ),
    })),
  );
