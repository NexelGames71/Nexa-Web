...
export function getPayPalPlanId(planKey) {
  ...
  const activeEnvironmentId = envValue(`PAYPAL_${paypalEnvironment}_${key}_PLAN_ID`);
  if (activeEnvironmentId) {
    return activeEnvironmentId;
  }

  if (key === "PLUS") {
    return envValue("PAYPAL_PLUS_PLAN_ID", "NEXT_PUBLIC_PAYPAL_PLUS_PLAN_ID");
  }

  return envValue(`PAYPAL_${key}_PLAN_ID`);
}

export function getPlanKeyForPayPalPlanId(paypalPlanId) {
  ...
  for (const plan of PAID_BILLING_PLANS) {
    if (getPayPalPlanId(plan.id) === target) {
      return plan.id;
    }
  }
...
export const PAYPAL_PLUS_PLAN_ID = getPayPalPlanId("plus");
